...
<template>
  <header class="fixed top-0 right-0 left-0 lg:left-64 z-30 bg-[#E8EDF2]/80 backdrop-blur-md">
    <div class="flex items-center justify-between px-4 sm:px-6 h-16">
      <button @click="toggleSidebar" class="lg:hidden neumorphic-raised p-2 rounded-xl">
        <svg class="w-6 h-6 text-[#4B5563]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      <div class="flex items-center gap-4">
        ...
        <div class="flex items-center gap-3 neumorphic-raised rounded-2xl px-4 py-2">
          ...
          <div class="hidden sm:block">
            <p class="text-sm font-medium text-[#1F2937]">{{ user?.name || 'Usuario' }}</p>
            <p class="text-xs text-[#6B7280] capitalize">{{ user?.rol || '' }}</p>
          </div>
        </div>
